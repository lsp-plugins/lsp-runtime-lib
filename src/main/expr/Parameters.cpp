...
namespace lsp
{
    namespace expr
    {
        ...
        ssize_t Parameters::get_index(const LSPString *name) const
        {
            for (size_t i=0, n=vParams.size(); i<n; ++i)
            {
                const param_t *p = vParams.uget(i);
                if ((p != NULL) && (p->len >= 0) && (name->equals(p->name, p->len)))
                    return i;
            }
        ...
        {
            const param_t *p = vParams.get(index);
            if (p == NULL)
        ...
        void Parameters::destroy_params(lltl::parray<param_t> &params)
        {
            for (size_t i=0; i<params.size(); ++i)
                destroy(params.uget(i));
            params.flush();
        }
        ...
        {
            const lltl::parray<param_t> &vp = p->vParams;
            ...
            // Perform a copy
            lltl::parray<param_t> slice;
            for ( ; first < last; ++first)
            {
                param_t *p = clone(vp.uget(first));
        ...
        {
            const param_t *v = vParams.get(index);
            if (v == NULL)
                return STATUS_INVALID_VALUE;
