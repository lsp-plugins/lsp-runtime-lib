...
namespace lsp
{
    namespace lspc
    {
        void AudioReader::decode_u8(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_s8(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_u16(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_s16(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_u24le(float *vp, const void *src, size_t ns)
        {
            const uint8_t *p = reinterpret_cast<const uint8_t *>(src);
            while (ns--)
            {
        ...
        void AudioReader::decode_u24be(float *vp, const void *src, size_t ns)
        {
            const uint8_t *p = reinterpret_cast<const uint8_t *>(src);
            while (ns--)
            {
        ...
        void AudioReader::decode_s24le(float *vp, const void *src, size_t ns)
        {
            const uint8_t *p = reinterpret_cast<const uint8_t *>(src);
            while (ns--)
            {
        ...
        void AudioReader::decode_s24be(float *vp, const void *src, size_t ns)
        {
            const uint8_t *p = reinterpret_cast<const uint8_t *>(src);
            while (ns--)
            {
        ...
        void AudioReader::decode_u32(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_s32(float *vp, const void *src, size_t ns)
        {
            const int32_t *p = reinterpret_cast<const int32_t *>(src);
            while (ns--)
            {
        ...
        void AudioReader::decode_f32(float *vp, const void *src, size_t ns)
        {
        ...
        void AudioReader::decode_f64(float *vp, const void *src, size_t ns)
        {
        ...
        status_t AudioReader::read_audio_header(ChunkReader *rd)
        {
            chunk_audio_header_t hdr;
            audio_parameters_t p;

            // Read audio header from chunk
            ssize_t res = rd->read_header(&hdr, sizeof(chunk_audio_header_t));
            if (res < 0)
                return status_t(-res);
            ...
            if (hdr.common.size < sizeof(chunk_audio_header_t))
                return STATUS_CORRUPTED_FILE;
        ...
        status_t AudioReader::apply_params(const audio_parameters_t *p)
        {
            if (p->channels <= 0)
        ...
        status_t AudioReader::open(File *lspc, bool auto_close)
        {
            if (nFlags & F_OPENED)
                return STATUS_OPENED;
            nFlags      = 0;

            ChunkReader *rd = lspc->find_chunk(LSPC_CHUNK_AUDIO, NULL, 0);
            if (rd == NULL)
        ...
        status_t AudioReader::open(File *lspc, uint32_t uid, bool auto_close)
        {
            if (nFlags & F_OPENED)
                return STATUS_OPENED;
            nFlags      = 0;

            ChunkReader *rd = lspc->read_chunk(uid);
            if (rd == NULL)
                return STATUS_NOT_FOUND;
        ...
        status_t AudioReader::open_raw_magic(File *lspc, const audio_parameters_t *params, uint32_t magic, bool auto_close)
        {
            if (nFlags & F_OPENED)
                return STATUS_OPENED;
            else if (params == NULL)
            ...
            ChunkReader *rd = lspc->find_chunk(magic, NULL, 0);
            if (rd == NULL)
                return STATUS_NOT_FOUND;
        ...
        status_t AudioReader::open_raw_uid(File *lspc, const audio_parameters_t *params, uint32_t uid, bool auto_close)
        {
            if (nFlags & F_OPENED)
                return STATUS_OPENED;
            else if (params == NULL)
        ...
        status_t AudioReader::open_raw(ChunkReader *rd, const audio_parameters_t *params, bool auto_close)
        {
            if (nFlags & F_OPENED)
                return STATUS_OPENED;
            else if (params == NULL)
        ...
        status_t AudioReader::get_parameters(audio_parameters_t *dst) const
        {
            if (!(nFlags & F_OPENED))
                return STATUS_CLOSED;
            else if (dst == NULL)
        ...
        status_t AudioReader::fill_buffer()
        {
        ...
        ssize_t AudioReader::read_samples(float **data, size_t frames)
        {
            if (!(nFlags & F_OPENED))
                return STATUS_CLOSED;
        ...
        ssize_t AudioReader::read_frames(float *data, size_t frames)
        {
            if (!(nFlags & F_OPENED))
                return STATUS_CLOSED;
        ...
        ssize_t AudioReader::skip_frames(size_t frames)
        {
            if (!(nFlags & F_OPENED))
                return STATUS_CLOSED;
        ...
        uint32_t AudioReader::unique_id() const
        {
            if (!(nFlags & F_OPENED))
                return 0;
            else if (pRD == NULL)
        ...
        uint32_t AudioReader::magic() const
        {
            if (!(nFlags & F_OPENED))
                return 0;
            else if (pRD == NULL)
